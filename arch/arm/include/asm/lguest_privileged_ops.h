...
#error "lguest_privileged_ops is no longer needed"
...
{
	avm_mmu_ops.__pmd_populate(pmdp, ((pte + PTE_HWTABLE_OFF) | prot));
}
...
{
	unsigned long rv;
	avm_irq_ops.m2f_raw_local_irq_save(&rv);
	return rv;
}
...
{
	unsigned long rv;
	avm_irq_ops.m2f_raw_local_save_flags(&rv);
	return rv;
}
