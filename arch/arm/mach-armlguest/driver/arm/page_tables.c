...
static pte_t *spte_addr(struct lg_cpu *cpu, pgd_t *spgd, unsigned long vaddr)
{
	...
	page =__va(ptr) + (pte_index(vaddr) * sizeof(void *));
		...
		unsigned long end)
{
	...
	do {
		...
		set_guest_pte(pte, pfn_pte(pfn,__pgprot((GUEST_BASE_PTE_FLAGS
				| L_PTE_DIRTY) &~ (L_PTE_RDONLY | L_PTE_XN))), 0);
...
void map_switcher_in_guest(struct lg_cpu *cpu, struct lguest_pages *pages)
{
	...
	regs_pte = pfn_pte(__pa(cpu->regs_page) >> PAGE_SHIFT, 
				__pgprot((GUEST_BASE_PTE_FLAGS | L_PTE_DIRTY) & ~L_PTE_RDONLY));
	index = PTRS_PER_PTE + pte_index((unsigned long)pages);
...
{
	unsigned int i;
	unsigned int index = PTRS_PER_PTE + pte_index((unsigned long)get_switcher_addr());
	...
	for (i = 0; i < pages; i++) {
		set_guest_pte(&pte[i], mk_pte(switcher_page[i],
                    __pgprot(GUEST_BASE_PTE_FLAGS & ~L_PTE_XN)),0);
	}
	...
	set_guest_pte(&pte[i], pfn_pte(page_to_pfn(switcher_page[i]),
			__pgprot((GUEST_BASE_PTE_FLAGS | L_PTE_DIRTY) & ~L_PTE_RDONLY)),0);
...
#define GUEST_VECTOR_PTE_FLAGS \
	(L_PTE_PRESENT | L_PTE_YOUNG | L_PTE_MT_WRITETHROUGH | L_PTE_USER | L_PTE_RDONLY)
	...
	set_guest_pte(&vectors_pte_page[PTRS_PER_PTE + pte_index(gvector_addr)], vector_pte,0);
