    ...
    struct page *from, unsigned long vaddr)
{
	...
	copy_page(kto, kfrom);
	lguest_flush_kern_dcache_area(kto, PAGE_SIZE);
...
static void disable_lguest_irq(unsigned int irq)
{
	...
	set_bit(irq, lgregs->blocked_interrupts);
}
...
{
	struct lguest_regs *lgregs = (struct lguest_regs *)lguest_page_base;
	clear_bit(irq, lgregs->blocked_interrupts);
...
static void __init lguest_init_irq(void)
{
	...
	for (i = 0; i < LGUEST_IRQS; i++) {
		set_irq_chip(i, &lguest_irq_chip);
...
static void __init lguest_fixup(struct machine_desc *desc,
