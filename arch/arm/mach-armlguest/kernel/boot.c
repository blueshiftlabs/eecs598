...
#ifndef CONFIG_ARM_LGUEST_GUEST
#error CONFIG_ARM_LGUEST_GUEST must be defined!
#endif
...
void lguest_setup_irq(unsigned int irq)
{
	...
	irq_set_chip_and_handler_name(irq, lg_irq_chip,
 ...
 */
static void lguest___pmd_populate(pmd_t *pmdp, phys_addr_t pte, pmdval_t prot)
{
 ...
 * processor state.  
 */
static unsigned long lguest_arch_local_save_flags(void)
{
	struct lguest_regs *lgregs = (struct lguest_regs *)lguest_page_base;

	return lgregs->irq_disabled;
}
 ...
 * processor state.  
 */
static unsigned long lguest_arch_local_irq_save(void)
{
	struct lguest_regs *lgregs = (struct lguest_regs *)lguest_page_base;
	unsigned long rv = lgregs->irq_disabled;
	lgregs->irq_disabled = PSR_I_BIT;
	return rv;
}
 ...
 */
static void lguest_arch_local_irq_restore(unsigned long x)
{
	struct lguest_regs *lgregs = (struct lguest_regs *)lguest_page_base;
...
/* Enable local irqs*/
static void lguest_arch_local_irq_enable(void)
{
	struct lguest_regs *lgregs = (struct lguest_regs *)lguest_page_base;
...
/* Interrupts go off... */
static void lguest_arch_local_irq_disable(void)
{
	struct lguest_regs *lgregs = (struct lguest_regs *)lguest_page_base;
...
static void lguest_flush_pmd_entry(void *pmd)
{ 
...
static void lguest_clean_pmd_entry(void *pmd)
{
 ...
 */
static void lguest___flush_icache_all(void)
{
...
extern struct task_struct *lguest___switch_to(struct task_struct *, struct thread_info *, struct thread_info *);
...
static void lguest_show_coproc_regs(char *buf)
{
	unsigned int ctrl;
...
static void lguest_syscall_set_tls(unsigned long tls)
{
 ...
 */
static int lguest___get_cpu_architecture(void)
{
	struct lguest_regs *lgregs = (struct lguest_regs *)lguest_page_base;
...
#define SET_HOOK(fn) do { \
	lguest_define_hook(fn); \
	fn = &lguest_##fn; \
} while (0)

	/* arch/arm/include/asm/cacheflush.h */
	SET_HOOK(__flush_icache_all);

	/* arch/arm/include/asm/cputype.h */
	SET_HOOK(read_cpuid);
	SET_HOOK(read_cpuid_ext);
	...
	SET_HOOK(arch_local_irq_save);
	SET_HOOK(arch_local_irq_enable);
	SET_HOOK(arch_local_irq_disable);
	SET_HOOK(local_fiq_enable);
	SET_HOOK(local_fiq_disable);
	SET_HOOK(arch_local_save_flags);
	SET_HOOK(arch_local_irq_restore);

	/* arch/arm/include/asm/mmu_context.h */
	SET_HOOK(enter_lazy_tlb);
	SET_HOOK(switch_mm);

	/* arch/arm/include/asm/pgalloc.h */
	SET_HOOK(__pmd_populate);
	...
	SET_HOOK(cpu_get_pgd);

	/* arch/arm/include/asm/processor.h */
	SET_HOOK(cpu_relax);
	SET_HOOK(__switch_to);

	/* arch/arm/include/asm/ptrace.h */
	SET_HOOK(user_mode);
	SET_HOOK(processor_mode);

	/* arch/arm/include/asm/system.h */
	SET_HOOK(get_cr);
	SET_HOOK(set_cr);
	SET_HOOK(get_copro_access);
	SET_HOOK(set_copro_access);

	/* arch/arm/include/asm/tlbflush.h */
	SET_HOOK(local_flush_tlb_all);
	SET_HOOK(local_flush_tlb_mm);
	SET_HOOK(local_flush_tlb_page);
	SET_HOOK(local_flush_tlb_kernel_page);
	SET_HOOK(flush_pmd_entry);
	SET_HOOK(clean_pmd_entry);

	/* arch/arm/kernel/traps.c */
	SET_HOOK(syscall_set_tls);
	SET_HOOK(early_trap_init);

	/* arch/arm/kernel/process.c */
	SET_HOOK(show_coproc_regs);
	SET_HOOK(kernel_thread_cpsr);

	/* arch/arm/kernel/setup.c */
	SET_HOOK(__get_cpu_architecture);
	SET_HOOK(cpu_tcm_init);
